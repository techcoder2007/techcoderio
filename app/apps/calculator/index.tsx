...
const Calculator = () => {
	...
	const calculate = useCallback(
		(a: number, b: number, op: OperationType): CalculationResult => {
			try {
				let result: number;
	...
	const handleUnaryOperation = useCallback(
		(op: OperationType, value: number): CalculationResult => {
			try {
				let result: number;
			...
			setState((prev) => {
				...
				switch (op) {
					...
					case OperationType.PAREN_CLOSE:
						newDisplay = prev.newNumber
							? OPERATIONS[op].symbol
							: prev.display + OPERATIONS[op].symbol;
